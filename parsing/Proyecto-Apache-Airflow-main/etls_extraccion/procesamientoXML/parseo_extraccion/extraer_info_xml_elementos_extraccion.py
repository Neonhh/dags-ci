...
def sql_optimizado(sql_text: str) -> str:
    ...
    3) Elimina el simbolo % al inicio de nombres de tabla (por ejemplo: %COL_XYZ → COL_XYZ)
    4) Reemplaza los placeholders :columna por %s para que Postgres los entienda
    5) Compacta los VALUES (...) en un solo VALUES %s (para usar con execute_values)
    """
    # 1) #conexion.var    →  {var}
    sql = re.sub(r"#\w+\.(\w+)", r"{\1}", sql_text)
    # 2) LS_ESQUEMA.tab  →  ESQUEMA.tab
    sql = re.sub(r"\bLS_([A-Za-z]\w*)\b", r"\1", sql)
    # 3) %nombre_tabla     →  nombre_tabla
    sql = re.sub(r"%(\w+)", r"\1", sql)
    # 4) :nombre_columna    →  %s
    sql = re.sub(r":\w+", "%s", sql)
    # 5) Simplificar bloque VALUES
    sql = re.sub(
        r"VALUES\s*\((\s*%s\s*,?[\s\r\n]*)+\)",  # detecta VALUES con muchos %s dentro
        "VALUES %s",
        sql,
...
def serializar_diccionario_arbol_package(pkg_info, output_filename):
    ...
    for step in SnpScenStep_list:
        ...
        if tareas:
            # 2) ordenamos las tareas por OrdTrt (Order del ODI)
            tareas = sorted(
                tareas,
                key=lambda t: int(t["OrdTrt"]) if isinstance(t.get("OrdTrt"), str) and t["OrdTrt"].isdigit() else 0
            )
...
# Ejecutar: python3 procesamientoXML/parseo_extraccion/extraer_info_xml_elementos_extraccion.py XMLs/EXTRACCION/AT26/PACKAGE_AT26_TO_FILE_ERROR.xml JSON/EXTRACCION/AT26/PACKAGE_AT26_TO_FILE_ERROR.json