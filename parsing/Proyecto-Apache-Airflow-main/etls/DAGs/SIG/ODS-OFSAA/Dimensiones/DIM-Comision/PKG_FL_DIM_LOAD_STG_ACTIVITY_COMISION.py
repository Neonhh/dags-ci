...
def vStgFechaCargaBan(**kwargs):
    ...
    Variable.set('vStgFechaCargaBan', result[0][0])
...
def IT_STG_ACTIVITY_B_INTF_ODS_STG(**kwargs):
    ...
    kwargs['ti'].log.info("Creando/verificando tabla")
    hook.run(sql_query_deftxt)
    kwargs['ti'].log.info("Tabla verificada/creada.")
...
def Actualizar_status_carga(**kwargs):
    ...
    vStgNombreTablaStg = Variable.get('vStgNombreTablaStg')
    vStgFormatoFecha = Variable.get('vStgFormatoFecha')
    vStgCargaStatus = Variable.get('vStgCargaStatus')
    vStgFechaCargaBan = Variable.get('vStgFechaCargaBan')
    ...
    kwargs['ti'].log.info("Accion a ejecutarse: OFSAA UPD BAN_CONF_STG_DATE Status Carga") 
    hook.run(sql_query_deftxt)
    kwargs['ti'].log.info("Accion: OFSAA UPD BAN_CONF_STG_DATE Status Carga, ejecutada exitosamente") 
