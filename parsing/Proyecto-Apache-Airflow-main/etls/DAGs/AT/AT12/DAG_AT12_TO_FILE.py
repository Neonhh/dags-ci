...
def AT12_ATSUDEBAN_TOFILE(**kwargs):
...
def ATS_TH_AT12_TOTXT(**kwargs):
    ...
    gcs_object_path = f"data/AT12/SALIDAS/{AT12_FileName_TDD}"
    
    temp_dir = tempfile.mkdtemp() # Crea un directorio temporal
    local_file_path = os.path.join(temp_dir, f"{AT12_FileName_TDD}") # Ruta del archivo temporal local
...
dag = DAG(dag_id='AT12_TO_FILE',
          default_args=default_args,
          schedule=None, # Aqui se programa cada cuanto ejecutar el DAG
          catchup=False)

AT12_ATSUDEBAN_TOFILE_task = PythonOperator(
    task_id='AT12_ATSUDEBAN_TOFILE_task',
    python_callable=AT12_ATSUDEBAN_TOFILE,
    dag=dag
)

ATS_TH_AT12_TOTXT_task = PythonOperator(
    task_id='ATS_TH_AT12_TOTXT_task',
    python_callable=ATS_TH_AT12_TOTXT,
    dag=dag
)
...
Enviar_Email_task = EmailOperator(
    ...
    html_content="""                             
        <h3>¡Hola!</h3>
        <p>El DAG <b>{{ dag.dag_id }}</b> finalizó correctamente, generando el reporte: {{ var.value.AT12_FileName_TDD }} </p>
    """,
    conn_id="email_conn",
	dag=dag
)
...
AT12_ATSUDEBAN_TOFILE_task >> ATS_TH_AT12_TOTXT_task >> Enviar_Email_task
