...
def FechaInicio_M(**kwargs):
	...
	sql_query = '''SELECT TO_CHAR(DATE_TRUNC('month', CURRENT_DATE - INTERVAL '4 months'), 'MM/DD/YYYY');''' # pruebas de mayo
	result = hook.get_records(sql_query)
...
def FechaFin_M(**kwargs):
	...
	sql_query = '''SELECT TO_CHAR(DATE_TRUNC('month', CURRENT_DATE - INTERVAL '4 months') + INTERVAL '1 month - 1 day', 'MM/DD/YYYY');''' # pruebas de mayo
	result = hook.get_records(sql_query)
