...
def AT13_ATSUDEBAN_ERROR(**kwargs):
	hook = PostgresHook(postgres_conn_id='repodataprd')
	
	sql_query_deftxt = '''CREATE TABLE IF NOT EXISTS FILE_AT.ATS_TH_AT13_ERROR (
	OFICINA VARCHAR(10),
	...
	hook.run("TRUNCATE TABLE FILE_AT.ATS_TH_AT13_ERROR;")
  
	# INSERTAR DATOS EN DESTINO
	sql_query_deftxt = '''INSERT INTO FILE_AT.ATS_TH_AT13_ERROR (
	OFICINA,
	TIPOCLIENTE,
	IDCLIENTE,
	FECHARECLAMO,
	...
	SELECT 
		...
		FRANQUICIA,
		RED
	FROM ATSUDEBAN.E$_ATS_TH_AT13;'''
	hook.run(sql_query_deftxt)
	
def ATS_TH_AT13_ERROR_TOTXT(**kwargs):
	...
	registros = hook.get_records("SELECT * FROM FILE_AT.ATS_TH_AT13_ERROR;")
	...
	gcs_object_path = f"data/AT13/SALIDAS/{FileName_Error}{FileAT}{FechaFile}.txt"
...
dag = DAG(dag_id='AT13_TO_FILE_ERROR', default_args=default_args, schedule=None, catchup=False)
...
AT13_ATSUDEBAN_ERROR_task = PythonOperator(
	task_id='AT13_ATSUDEBAN_ERROR_task',
	python_callable=AT13_ATSUDEBAN_ERROR,
	dag=dag
)

ATS_TH_AT13_ERROR_TOTXT_task = PythonOperator(
	task_id='ATS_TH_AT13_ERROR_TOTXT_task',
	python_callable=ATS_TH_AT13_ERROR_TOTXT,
	dag=dag
)

###### SECUENCIA DE EJECUCION ######
FileName_Error_task >> AT13_ATSUDEBAN_ERROR_task >> ATS_TH_AT13_ERROR_TOTXT_task
