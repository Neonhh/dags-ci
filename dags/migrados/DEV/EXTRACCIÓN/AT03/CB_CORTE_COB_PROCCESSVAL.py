...
def PROC_UPD_CONFIG_ODS(**kwargs):
	...
	SET COO_END_DATE = TO_DATE(CASE WHEN '{vOdsFechaFin}' = '01/01/1801' THEN '{vOdsFechaInicio}' ELSE '{vOdsFechaFin}' END, '{vSrcFormatoFecha}'),
...
def vSrcFechaCarga(**kwargs):
	...
	sql_query = f''' SELECT
	COALESCE(TO_CHAR(MIN(bcs.cds_date), '{vSrcFormatoFecha}'),'01/01/1801')
	FROM ods.ban_config_ods_date AS bcs JOIN ods.ban_config_ods AS o ON o.coo_source = bcs.cds_source
...
def PROC_UPD_STATUS_BAN_CONFIG_ODS(**kwargs):
	...
	BEGIN
		...
		FROM ods.BAN_CONFIG_ODS_DATE AS bcsd, ods.BAN_CONFIG_ODS AS bcs
...
dag = DAG(dag_id='PKG_CON_PROCCESSVAL_CB_CORTE_COB_CONTA_ODS',
...
PKG_CON_LOAD_CB_CORTE_COB_CONTA_ODS_task = TriggerDagRunOperator(
	task_id='PKG_CON_LOAD_CB_CORTE_COB_CONTA_ODS_task',
	trigger_dag_id='PKG_CON_LOAD_CB_CORTE_COB_CONTA_ODS',
...
vSrcNombreTablaOds_task >> vOdsFechaInicio_task >> vOdsFechaFin_task >> vSrcFormatoFecha_task >> PROC_UPD_CONFIG_ODS_task >> vSrcFechaCarga_task >> PKG_CON_LOAD_CB_CORTE_COB_CONTA_ODS_task >> PROC_UPD_STATUS_BAN_CONFIG_ODS_task
