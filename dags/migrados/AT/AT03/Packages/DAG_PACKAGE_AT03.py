...
WHERE
    ...
    # Load data
    sql_query_deftxt = '''INSERT INTO "W"."%COL_PRF0AGRUPACION_BANCARIBE" (C1_OFICINA, C2_CUENTA, C3_SALDO, C4_EMPRESA, C5_CATEGORIA) VALUES (:C1_OFICINA, :C2_CUENTA, :C3_SALDO, :C4_EMPRESA, :C5_CATEGORIA);'''
    logger.info("Accion a ejecutarse: Load data")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Load data, ejecutada exitosamente")

    # Create target table
    sql_query_deftxt = '''CREATE TABLE IF NOT EXISTS "AGRUPACION_BANCARIBE" (
    OFICINA NUMERIC(10),
    CUENTA VARCHAR(12),
    SALDO NUMERIC(32, 7),
    EMPRESA NUMERIC(3),
...
WHERE (1 = 1);'''
    ...
    # Create target table
    sql_query_deftxt = '''CREATE TABLE IF NOT EXISTS AT03_V001_BANCARIBE (
    OFICINA VARCHAR(4),
    CUENTA VARCHAR(12),
    SALDO NUMERIC(32, 7),
    CONSTANTE VARCHAR(1)
    ...
    # Truncate target table
    sql_query_deftxt = '''TRUNCATE TABLE AT03_V001_BANCARIBE;'''
    logger.info("Accion a ejecutarse: Truncate target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Truncate target table, ejecutada exitosamente")

    # Insert new rows
    sql_query_deftxt = '''INSERT INTO AT03_V001_BANCARIBE (CUENTA, SALDO, OFICINA, CONSTANTE)
...
WHERE
    ...
    # Load data
    sql_query_deftxt = '''INSERT INTO "W"."%COL_PRF0CB_HIST_SALDO_AT03" (C1_HI_EMPRESA, C2_HI_CUENTA, C3_HI_OFICINA, C4_HI_AREA, C5_HI_CORTE, C6_HI_PERIODO, C7_HI_SALDO, C8_HI_SALDO_ME) VALUES (:C1_HI_EMPRESA, :C2_HI_CUENTA, :C3_HI_OFICINA, :C4_HI_AREA, :C5_HI_CORTE, :C6_HI_PERIODO, :C7_HI_SALDO, :C8_HI_SALDO_ME);'''
    logger.info("Accion a ejecutarse: Load data")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Load data, ejecutada exitosamente")

    # Truncate target table
    sql_query_deftxt = '''TRUNCATE TABLE CB_HIST_SALDO_AT03;'''
    logger.info("Accion a ejecutarse: Truncate target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Truncate target table, ejecutada exitosamente")

    # Delete target table
    sql_query_deftxt = '''DELETE FROM CB_HIST_SALDO_AT03;'''
    logger.info("Accion a ejecutarse: Delete target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Delete target table, ejecutada exitosamente")

    # Insert new rows
    sql_query_deftxt = '''INSERT INTO CB_HIST_SALDO_AT03 (HI_EMPRESA, HI_CUENTA, HI_OFICINA, HI_AREA, HI_CORTE, HI_PERIODO, HI_SALDO, HI_SALDO_ME, HI_FECHA_INI, HI_FECHA_FIN)
...
WHERE
    ...
    # Load data
    sql_query_deftxt = '''INSERT INTO "W"."%COL_PRF0CB_HIST_SALDO_AT" (C1_HI_EMPRESA, C2_HI_CUENTA, C3_HI_OFICINA, C4_HI_AREA, C5_HI_CORTE, C6_HI_PERIODO, C7_HI_SALDO, C8_HI_SALDO_ME) VALUES (:C1_HI_EMPRESA, :C2_HI_CUENTA, :C3_HI_OFICINA, :C4_HI_AREA, :C5_HI_CORTE, :C6_HI_PERIODO, :C7_HI_SALDO, :C8_HI_SALDO_ME);'''
    logger.info("Accion a ejecutarse: Load data")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Load data, ejecutada exitosamente")

    # Truncate target table
    sql_query_deftxt = '''TRUNCATE TABLE CB_HIST_SALDO_AT;'''
    logger.info("Accion a ejecutarse: Truncate target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Truncate target table, ejecutada exitosamente")

    # Delete target table
    sql_query_deftxt = '''DELETE FROM CB_HIST_SALDO_AT;'''
    logger.info("Accion a ejecutarse: Delete target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Delete target table, ejecutada exitosamente")

    # Insert new rows
    sql_query_deftxt = '''INSERT INTO ODS.CB_HIST_SALDO_AT (HI_EMPRESA, HI_CUENTA, HI_OFICINA, HI_AREA, HI_CORTE, HI_PERIODO, HI_SALDO, HI_SALDO_ME, HI_FECHA_INI, HI_FECHA_FIN)
...
WHERE (1 = 1);'''
    ...
    # Create target table
    sql_query_deftxt = '''CREATE TABLE IF NOT EXISTS AT03_V001_BANCARIBE (OFICINA VARCHAR(4), CUENTA VARCHAR(12), SALDO NUMERIC(32, 7), CONSTANTE VARCHAR(1));'''
    logger.info("Accion a ejecutarse: Create target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Create target table, ejecutada exitosamente")
...
WHERE
    ...
    # Insert new rows
    sql_query_deftxt = '''INSERT INTO CB_HIST_SALDO_AT (HI_EMPRESA, HI_CUENTA, HI_OFICINA, HI_AREA, HI_CORTE, HI_PERIODO, HI_SALDO, HI_SALDO_ME, HI_FECHA_INI, HI_FECHA_FIN)
SELECT HI_EMPRESA, HI_CUENTA, HI_OFICINA, HI_AREA, HI_CORTE, HI_PERIODO, HI_SALDO, HI_SALDO_ME, NOW(), NOW()
FROM (SELECT C1_HI_EMPRESA, C2_HI_CUENTA, C3_HI_OFICINA, C4_HI_AREA, C5_HI_CORTE, C6_HI_PERIODO, C7_HI_SALDO, C8_HI_SALDO_ME
...
WHERE
    ...
    # Delete target table
    sql_query_deftxt = '''DELETE FROM agrupacion_bancari;'''
    logger.info("Accion a ejecutarse: Delete target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Delete target table, ejecutada exitosamente")
...
WHERE (1 = 1);'''
    ...
    # Insert new rows
    sql_query_deftxt = '''INSERT INTO AT03_V001_BANCARIBE (CUENTA, SALDO, OFICINA, CONSTANTE)
SELECT CUENTA, SUM(SALDO), 'V001', '1'
FROM AT03_OFICINA_FINAL
WHERE (1=1)
GROUP BY CUENTA;'''
    logger.info("Accion a ejecutarse: Insert new rows")
...
WHERE
    ...
    # Delete target table
    sql_query_deftxt = '''DELETE FROM agrupacion_bancaribe;'''
    logger.info("Accion a ejecutarse: Delete target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Delete target table, ejecutada exitosamente")
...
WHERE
    ...
    # Load data
    sql_query_deftxt = '''INSERT INTO "%COL_PRF0AGRUPACION_BANCARIBE" (C1_OFICINA, C2_CUENTA, C3_SALDO, C4_EMPRESA, C5_CATEGORIA) VALUES (:C1_OFICINA, :C2_CUENTA, :C3_SALDO, :C4_EMPRESA, :C5_CATEGORIA);'''
    logger.info("Accion a ejecutarse: Load data")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Load data, ejecutada exitosamente")
...
def IT_CB_HIST_SALDO_ODS_BANCARIBE_AT03_ODS(**kwargs):
    ...
    # Insert new rows
    sql_query_deftxt = f'''INSERT INTO ODS.CB_HIST_SALDO_AT03 (HI_EMPRESA, HI_CUENTA, HI_OFICINA, HI_AREA, HI_CORTE, HI_PERIODO, HI_SALDO, HI_SALDO_ME, HI_DEBITO, HI_CREDITO, HI_DEBITO_ME, HI_CREDITO_ME, HI_FECHA_INI, HI_FECHA_FIN)
SELECT HI_EMPRESA,
       LTRIM(RTRIM(HI_CUENTA)) AS HI_CUENTA,
       HI_OFICINA,
       HI_AREA,
       HI_CORTE,
       HI_PERIODO,
...
def AT03_AGRUPACION_CONTABLE_BANCARIBE(**kwargs):
    ...
    sql_query_deftxt = '''INSERT INTO "AGRUPACION_BANCARIBE" (OFICINA, CUENTA, SALDO, EMPRESA, CATEGORIA)
SELECT DISTINCT C1_OFICINA, C2_CUENTA, C3_SALDO, C4_EMPRESA, C5_CATEGORIA
FROM "%COL_PRF0AGRUPACION_BANCARIBE" AS "AGRUPACION_BANCARIBE"
WHERE (1 = 1);'''
    logger.info("Accion a ejecutarse: Insert new rows")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Insert new rows, ejecutada exitosamente")
...
def AT03_AGRUPACION_OFICINAS_BANCARIBE(**kwargs):
    ...
    # Create target table
    sql_query_deftxt = '''CREATE TABLE IF NOT EXISTS AT03_OFICINA (CUENTA VARCHAR(12), OFICINA VARCHAR(4), SALDO NUMERIC(32, 7), CONSTANTE VARCHAR(1), EMPRESA NUMERIC(3), CATEGORIA VARCHAR(1));'''
    logger.info("Accion a ejecutarse: Create target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Create target table, ejecutada exitosamente")

    # Truncate target table
    sql_query_deftxt = '''TRUNCATE TABLE AT03_OFICINA;'''
    logger.info("Accion a ejecutarse: Truncate target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Truncate target table, ejecutada exitosamente")

    # Insert new rows
    sql_query_deftxt = '''INSERT INTO AT03_OFICINA (CUENTA, OFICINA, SALDO, EMPRESA, CATEGORIA, CONSTANTE)
SELECT
    CUENTA,
    LPAD(OFICINA, 4, '0'),
    SUM(SALDO),
...
FROM (
    SELECT
        ...
        EMPRESA,
        CATEGORIA
    FROM AGRUPACION_BANCARIBE
    INNER JOIN SIIF_AT03_CUENTACONTABLE ON TRIM(SIIF_AT03_CUENTACONTABLE.CUENTACONTABLE) = TRIM(AGRUPACION_BANCARIBE.CUENTA) AND TRIM(AGRUPACION_BANCARIBE.EMPRESA) = TRIM(SIIF_AT03_CUENTACONTABLE.IDBANCO)
    WHERE OFICINA NOT IN ('157', '306')
    GROUP BY CUENTA, OFICINA, EMPRESA, CATEGORIA
) AS ODI_GET_FROM;'''
    logger.info("Accion a ejecutarse: Insert new rows")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Insert new rows, ejecutada exitosamente")
...
def AT03_AGRUPACION_OFICINAS_BANCARIBE_SOBREGIRADA(**kwargs):
    ...
    # Create target table
    sql_query_deftxt = '''CREATE TABLE IF NOT EXISTS AT03_OFICINA_SOBREG (CUENTA VARCHAR(12), OFICINA VARCHAR(4), SALDO NUMERIC(32, 7), CONSTANTE VARCHAR(1), EMPRESA NUMERIC(3));'''
    logger.info("Accion a ejecutarse: Create target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Create target table, ejecutada exitosamente")

    # Truncate target table
    sql_query_deftxt = '''TRUNCATE TABLE AT03_OFICINA_SOBREG;'''
    logger.info("Accion a ejecutarse: Truncate target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Truncate target table, ejecutada exitosamente")

    # Insert new rows
    sql_query_deftxt = '''INSERT INTO AT03_OFICINA_SOBREG (CUENTA, SALDO, EMPRESA, OFICINA, CONSTANTE)
...
def AT03_AGRUPACION_OFICINAS_BANCARIBE_FINAL(**kwargs):
    ...
    # Create target table
    sql_query_deftxt = '''CREATE TABLE IF NOT EXISTS AT03_OFICINA_FINAL (CUENTA VARCHAR(12), OFICINA VARCHAR(4), SALDO NUMERIC(32,7), CONSTANTE VARCHAR(1), EMPRESA NUMERIC(3));'''
    logger.info("Accion a ejecutarse: Create target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Create target table, ejecutada exitosamente")

    # Truncate target table
    sql_query_deftxt = '''TRUNCATE TABLE AT03_OFICINA_FINAL;'''
    logger.info("Accion a ejecutarse: Truncate target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Truncate target table, ejecutada exitosamente")

    # Insert new rows
    sql_query_deftxt = '''INSERT INTO AT03_OFICINA_FINAL (CUENTA, OFICINA, SALDO, EMPRESA, CONSTANTE)
...
def ATS_TT_AT03(**kwargs):
    ...
    # Truncate target table
    sql_query_deftxt = '''TRUNCATE TABLE ATS_TH_AT03;'''
    logger.info("Accion a ejecutarse: Truncate target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Truncate target table, ejecutada exitosamente")

    # Delete target table
    sql_query_deftxt = '''DELETE FROM ATS_TH_AT03;'''
    logger.info("Accion a ejecutarse: Delete target table")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Delete target table, ejecutada exitosamente")

    # Insert new rows
    sql_query_deftxt = '''INSERT INTO ATS_TH_AT03 (OFICINA, CODIGO_CONTABLE, SALDO, CONSECUTIVO)
SELECT
    OFICINA,
    CODIGO_CONTABLE,
    SALDO,
...
WHERE
    ...
    # Load data
    sql_query_deftxt = '''INSERT INTO "%COL_PRF0CB_HIST_SALDO_AT" (C1_HI_EMPRESA, C2_HI_CUENTA, C3_HI_OFICINA, C4_HI_AREA, C5_HI_CORTE, C6_HI_PERIODO, C7_HI_SALDO, C8_HI_SALDO_ME) VALUES (:C1_HI_EMPRESA, :C2_HI_CUENTA, :C3_HI_OFICINA, :C4_HI_AREA, :C5_HI_CORTE, :C6_HI_PERIODO, :C7_HI_SALDO, :C8_HI_SALDO_ME);'''
    logger.info("Accion a ejecutarse: Load data")
    hook.run(sql_query_deftxt)
    logger.info("Accion: Load data, ejecutada exitosamente")
